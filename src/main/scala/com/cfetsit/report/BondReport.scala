...
object BondReport {
  ...
  def Ins_Show_Name_Transformation(Ins_Show_Name:String): String ={
    var name = new Array[String](100)

    if (Ins_Show_Name.contains("(")) name = Ins_Show_Name.split("\\(")
    else if (Ins_Show_Name.contains("（"))
      name = Ins_Show_Name.split("（")
   ...
   */
    ...
    //<买方机构ID,债券类型【8分类】,交易策略（数据源）,交易日期,债券ID> <买入全价金额>
    //<卖方机构ID,债券类型【8分类】,交易策略（数据源）,交易日期,债券ID> <卖出全价金额>
    //<机构ID,交易策略（数据源）,交易日期,债券ID> <成交金额>
    val modify_buyer_day7 = buyer_day7
      .map(x=>{
        ((x._1._1,x._1._3,x._1._4,x._1._5),x._2)
      })
    val modify_seller_day7 = seller_day7
      .map(x=>{
        ((x._1._1,x._1._3,x._1._4,x._1._5),x._2)
      })
    val union_info_for_bond_concentration = (modify_buyer_day7 union modify_seller_day7)
      .reduceByKey((x,y)=>(x+y))
      .map(record=>{
        //val oracleDateFormat = new SimpleDateFormat("yyyy-mm-dd")
        //val deal_date = oracleDateFormat.parse(record._1._3)
        val deal_date = record._1._3
        (record._1._1,record._1._2,deal_date,record._1._4,record._2.bigDecimal)
      })
      .saveToPhoenix("UNION_INFO_FOR_BOND_CONCENTRATION",Seq("MEMBER_ID","DEAL_STRATEGY","DEAL_DT","BOND_ID","FULL_PRICE"),conf,Some(ZOOKEEPER_URL))

    log.info("union_info_for_bond_concentration save success!")
    println("union_info_for_bond_concentration save success!")
  ...
  */
   ...
   */
    ...
    val root_roi_performance = (buyer_day9.fullOuterJoin(seller_day9))
      ...
      .map(x=>{
        //<机构ID,债券ID，交易日期，债券类型【4分类】>  <（买入全价金额,卖出全价金额,买入券面金额,卖出券面金额）>
        Tuple2(Tuple4(x._1._1,x._1._2,x._1._3,x._1._4),Tuple4(x._2._1._1,x._2._2._1,x._2._1._2,x._2._2._2))
      })
      .flatMap(record=>{
        //将机构ID映射为父机构ID
    ...
    */
    ...
    <买方机构ID,债券ID，债券类型【4分类】,交易日期> <买入全价金额>
    <卖方机构ID,债券ID，债券类型【4分类】,交易日期> <卖出全价金额>
    <机构ID,债券ID，债券类型【4分类】,交易日期>  <买入全价金额+卖出全价金额>
    cache() 复用,root_bond_concentration计算用到
     */

    val Bond_Concentration = (buyer_day10 union seller_day10)
      .reduceByKey((x,y)=>(x+y))
      .cache()
     ...
     */
    val root_bond_concentration = Bond_Concentration
      .flatMap(record=>{
        //将机构ID映射为父机构ID
        val member_id = record._1._1
        val member_uniq = broadMEMBER_D.value.get(member_id).get._9
